...
- **レポートMD（調査・分析・施設検索系）は push 後に必ず GitHub ハイパーリンクを3列表で報告する。ユーザーがリポジトリ構造を知らなくてもワンクリックで開ける状態を保証すること。**
...
**🔗 ワンクリック原則（最重要）**
報告リンクは**ブラウザでクリックしただけで該当ファイルが開ける**こと。
- レポート系MD（調査・施設検索・分析）は作成・更新・push後、**必ず即GitHubリンクで3列表報告**。
- ユーザーがリポジトリ構造を知らなくてもアクセス可能な状態を保証する。
