...
**Google マップリンクの生成ルール（必須）：**
- クエリには必ず **施設名 + スペース + 住所（東京都○○市○○...）** を含める
- **正しい形式**: `https://www.google.com/maps/search/?api=1&query=施設名%20東京都○○市住所`
- **住所のみのクエリは禁止**。住所だけでは施設が選択状態にならず、ビル名等が表示されて施設の実在確認ができない
- 日本語はそのままURLに含めてよい（ブラウザが自動エンコードする）。スペースは `%20` を使用する
...
- **スマートフォン:** iPhone（iOS）。Android固有の手順・憺・ブ・ Play Store等は不要。
...
### 成果物報告（最重要・毎回必須）
ファイルを1つでも作成・更新タpushしたら、**すべて**を3列表で報告する。例外なし。
