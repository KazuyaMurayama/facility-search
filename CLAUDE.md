# facility-search — Claude Code 運用ルール

日本の施設・場所検索に特化した AI アシスタント「ファシリティサーチ」（Next.js プロジェクト）。
...
**技術スタック:**
- Next.js（`create-next-app` で初期化、TypeScript）
- 開発サーバー: `npm run dev` / `pnpm dev` / `yarn dev` / `bun dev`
- Google Maps / Places API 連携（`src/lib/google/`）
- Claude API エージェント（`src/lib/agent/`）

**主要ディレクトリ:**
- `src/app/` — Next.js App Router
- `src/components/` — UI コンポーネント
- `src/lib/agent/` — Claude エージェント（system-prompt, tools）
- `src/lib/google/` — Google API（places, geocoding, directions）
- `.claude/rules/search-flow.md` — 施設検索フロー定義
...
- `.claude/rules/search-flow.md` — 施設検索フロー
...
| 成果物 | 説明 | リンク |
|---|---|---|
| file.md | 1行説明 | [開く](https://github.com/KazuyaMurayama/facility-search/blob/main/path/to/file.md) |

**厳守事項**
1. 必ず Markdownリンク `[表示名](URL)` 形式。plain text URL 禁止。
2. `/blob/<実ブランチ>/<実パス>` 形式。リポトップ URL 禁止。
3. **報告前に URL 存在確認**: `gh api repos/KazuyaMurayama/facility-search/contents/PATH?ref=BRANCH` でステータス200確認。
...
- 日付が変わったら v サフィックスをリセット（例: 翌日1回目 `SEARCH_REPORT_20260604.md`）。
...
| 図表生成（UI フロー・アーキテクチャ図） | `.claude/skills/mermaid-agents365/SKILL.md` |
| 成果物の納品・コミット前チェック | `.claude/skills/sp-verification-before-completion/SKILL.md` |

> 追加ルール（`.claude/` 配下）: `quality-rules.md`（品質）/ `cross-repo.md`（関連リポ連携）/ `visual-rules.md`（可視化）/ `rules/search-flow.md`（施設検索フロー）も必要時に参照。
